...
left-hand panel. Your project will be added to a list of your favorite projects.

In the future if you ever want to quickly get back to this project, open up
this panel and left-click your project to jump to it. You can also right-click
a project to open that project in a new window which is very convenient if
you like to reference code from previous projects when starting a new project.

## 2. GitLens

IMAGE: 02_gitlens.png

Gitlens is a great tool for examining when a line of code was changed and why.
When you click on a line of code, off to the right-hand side, you'll see
an un-obtrusive git blame message with (1) **who** changed that line of code last,
(2) **how long ago** it was changed, and (3) the **commit message** for that
change. If you then hover that git blame segment you'll have options to pull
up a `diff` file with all the changes made to the current file for that commit,
...
You can right click the file itself in the left-hand file tree or right click
...
I type fast and make frequent small spelling mistakes that often used to go
un-noticed until a co-worker pointed them out in code review. Well, no more!
As the name suggests, `Code Spell Checker` checks your code for spelling mistakes.
And not just spelling mistakes in comments, but in variable, function and
class names as well, with smart word tokenizing on `snake_case` and `camelCase`
code. Misspelled words are underlined, and if you click the underlined word a
lightbulb will appear on the left side of your screen with suggestions for
words with similar spellings. And if you right-click the word you can tell
the extension to add the word to the dictionary or ignore the word for now.
...
`(parentheses)` colors that match on the left and right side.
Inner brackets will get a different color than the outer brackets. This
coloring makes it so quick and easy to tell which brackets align on the
left and right side, and makes it very easy to tell when you're missing
a closing bracket.
...
`Toggle Quotes` is a fun, little extension that just allows you to quickly
...
If you are like me and work on a `Windows` machine but use `git-bash` or
have a project with file paths that need forward-slashes, and need your
copied file paths to also have forward slashes, `Copy relative path posix`
will save you so much time. When right-clicking a file in the left-hand file
tree, in addition to the standard "Copy Path" and "Copy Relative Path",
there will now be an option to "Copy Relative Path (Posix)" which will
...
and place your cursor between the opening and closing tags. It's also smart
enough to know certain tags that should not receive a closing tag
(such as `<br>`), and that list of non-closed tags can be extended.
...
If you edit the opening tag from that pair, it will change the closing tag to mach.
...
`Color Highlight` highlights all CSS color codes in your code base the color
...
`Sourcery` is a really cool extension for `python`. It finds patterns in
your opened python file that could be refactored to be made clearer,
and it underlines the affected code. If you hover the underlined code,
...
refactoring or even tell `Sourcery` to never show the suggested type of
refactoring again.
...
On the `Flask` side of things you can start typing `fapp`, as you type
...
And so on, there are 18 such basic flask snippets for quick code completion
...
These last three extensions aren't so much extensions as they are categories
...
language. Here are a list of the languages I use all the time alongside
...
`icon themes`. Icon themes transform the icons in the left-hand file tree
browser, making them "pop" a little more (or less). While the default theme
for VS Code icons (called `Seti`) is actually pretty nice looking, its
...
icons look in their description pages. I picked up the `material icon theme`.
...
found a couple new ones to pick up for yourself. Did I miss any VS Code
